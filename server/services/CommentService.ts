...
const commentUserModel = require("../models/UserModel.ts");
// 用户与评论是一对多关系
// commentUserModel.hasMany(CommentModel);  // 不要加这句，哪个模型需要关联就写一句声明
// 建立关联关系时，如果外键关联的是主键则不用写 targetKey，否则需要。
CommentModel.belongsTo(commentUserModel, {
  foreignKey: "email",
  targetKey: "email",
});
